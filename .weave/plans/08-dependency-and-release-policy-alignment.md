...
- No inspected tests enforce release checklist, dependency policy, catalog version alignment, or security review gates.
...
- [ ] 8. Define release token and workflow safety policy
  **What**: Document least-privilege release token requirements, prohibit token/secret logging, restrict workflow permissions to the minimum needed, require protected release/tag workflow expectations, and require dry-run plus manual verification before publishing.
  **Files**: `docs/releasing.md`, `.github/workflows/*`
  **Acceptance**: Maintainers can verify tokens are scoped only to publishing needs, workflows cannot publish from unprotected refs/tags, secrets are never printed, default permissions are restricted, and a dry-run/manual approval step precedes publishing.
