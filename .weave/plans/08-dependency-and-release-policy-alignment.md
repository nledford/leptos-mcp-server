...
- [x] 1. Inventory current policy and drift points
...
- [x] 2. Define dependency version policy
...
- [x] 3. Define embedded catalog version policy
...
- [x] 4. Add version alignment tests/checks
...
- [x] 5. Add release checklist gates
...
- [x] 6. Add security-sensitive review marker
...
- [x] 7. Capture breaking-change policy
...
- [x] 8. Define release token and workflow safety policy
...
- [x] Run any newly documented dependency audit/check command if adopted.
- [x] Run `cargo fmt -- --check` if Rust source/tests or workflow-adjacent generated files are changed.
- [x] Run `cargo clippy --locked --all-targets -- -D warnings` if Rust source/tests are changed.
- [x] Run `cargo test` before merging.
- [x] Fix any discovered or introduced test failures, compilation/type errors, Clippy warnings, formatting failures, audit/deny failures, or workflow validation failures in affected areas before marking this plan complete.
